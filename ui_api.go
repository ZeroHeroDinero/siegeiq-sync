...
func apiSaveSettings(raw string) string {
	...
	if err := json.Unmarshal([]byte(raw), &incoming); err != nil {
		// The failure sound too, not just the message. Somebody who clicked Save
		// and looked away should not be told it worked by silence.
		beep(false)
		return "could not read those settings: " + err.Error()
	}
	incoming.normalise()
	...
	// The quieter of the two sounds. Saving a setting is the app doing as it was
	// told, not an event worth interrupting anyone for, and it follows the same
	// notification switch as everything else - somebody who turned sounds off in
	// the tray still gets silence here.
	beepClip(true)
	return ""
}
