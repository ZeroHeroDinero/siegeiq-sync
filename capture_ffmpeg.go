...
// app down with it.
//
// THREE CAPTURE PATHS, IN ORDER OF PREFERENCE
//
//  1. gfxcapture - WinRT Windows Graphics Capture, pointed at Siege's own
//     window handle. It captures that window's composited output whether or
//     not it is the window in front, which is what lets recording carry on
//     while the player is in Discord or a browser. Nothing outside the game
//     window can reach a frame, so there is no crop and no way for the desktop
//     to leak into a clip even in borderless. Needs Windows 10 2004 or newer
//     and an FFmpeg built with the source, so it is asked for rather than
//     assumed - see probeGfxCapture.
//
//     It cannot capture a MINIMIZED window, which is what exclusive fullscreen
//     does on alt-tab. Borderless is the mode this helps.
//
//  2. ddagrab - DXGI Desktop Duplication. GPU-side, low overhead, works with
...
//     plays) the crop is the whole display and costs nothing. Because it hands
//     back the desktop, recording MUST pause when Siege is not in front.
//
//  3. gdigrab with title= - true single-window capture, but CPU-side and
...
type ffmpegCaps struct {
	...
	// The WinRT window-native path. Every one of these is MEASURED, not read
	// off a feature list, for the reason spelled out above encoderUsable.
	// GfxOpts records which option names this binary's own help text listed;
	// GfxHWFrames whether frames arrive on the GPU and need pulling down;
	// GfxBorderOff and GfxCursorOff whether those two options were accepted
	// when actually passed. GfxWhy carries the reason it is unavailable in
	// words that can be shown to a player.
	HasGfxCapture bool
	GfxHWFrames   bool
	GfxBorderOff  bool
	GfxCursorOff  bool
	GfxOpts       map[string]bool
	GfxWhy        string
	...
	// HasSegmentList is whether this build's segment muxer offers a segment
	// list, which is what gives segindex.go its microsecond-exact spacing.
	// Probed rather than assumed: these are the only options this app adds to
	// the recording command line that it does not otherwise test, and an ffmpeg
	// handed an option it does not know exits rather than carrying on.
	HasSegmentList bool
...
func warmCaptureCaps(rc recorderConfig) {
	warmOnce.Do(func() {
		...
		go func() {
			...
			if caps != nil {
				logf("recorder: capture engine ready - %s (window-native grab: %v, GPU grab: %v, "+
					"nvenc: %v, amf: %v, qsv: %v)",
					caps.Version, caps.HasGfxCapture, caps.HasDdagrab,
					caps.HasNVENC, caps.HasAMF, caps.HasQSV)
				if !caps.HasGfxCapture && caps.GfxWhy != "" {
					logf("recorder: recording will still pause on alt-tab - %s", caps.GfxWhy)
				}
			} else {
...
func probeFFmpeg(path string) (*ffmpegCaps, error) {
	...
	probeGfxCapture(path, c)
	if out, err := runFFmpegText(path, "-hide_banner", "-h", "muxer=segment"); err == nil {
		c.HasSegmentList = strings.Contains(out, "segment_list_flags") &&
			strings.Contains(out, "segment_list_type")
	}
...
// probeGfxCapture answers three separate questions about the WinRT window
// capture source, and asks the binary rather than assuming any of them.
//
// The source is recent enough that its option NAMES cannot be taken on trust,
// so they are read out of this ffmpeg's own help text and anything absent is
// simply never passed. Then it is run for two frames against a display, which
// needs no Siege window, to find out whether frames arrive on the GPU: passing
// hwdownload on system frames is a hard error and so is omitting it on GPU
// frames, so exactly one of the two attempts survives.
//
// Cost is up to four short launches, once, on the warm-up goroutine. That is
// the same trade encoderUsable makes, and for the same reason: a guess that
// looks like a fact is what produces a recorder that starts and records black.
func probeGfxCapture(path string, c *ffmpegCaps) {
	out, err := runFFmpegText(path, "-hide_banner", "-h", "filter=gfxcapture")
	if err != nil || strings.Contains(out, "Unknown filter") ||
		!strings.Contains(out, "hwnd") {
		c.GfxWhy = "this ffmpeg has no usable gfxcapture source"
		return
	}
	// Read off ffmpeg 9.0's own option table on 22 August 2026. The first draft
	// of this file guessed capture_hwnd, capture_hmonitor and frame_rate, and
	// every one of those guesses was wrong - the real names are below. The guess
	// cost nothing only because this check exists, which is the argument for it.
	c.GfxOpts = map[string]bool{}
	for _, name := range []string{"hwnd", "monitor_idx", "capture_cursor",
		"display_border", "max_framerate", "output_fmt"} {
		if strings.Contains(out, name) {
			c.GfxOpts[name] = true
		}
	}
	if !c.GfxOpts["monitor_idx"] {
		// Without a display target there is nothing to rehearse against, and a
		// rehearsal against the live game window is not something to do while
		// somebody is playing. Leave the path off rather than find out mid-round.
		c.GfxWhy = "gfxcapture cannot be tested on this build"
		return
	}

	twoFrames := func(chain string) bool {
		_, err := runFFmpegText(path, "-hide_banner", "-loglevel", "error",
			"-filter_complex", chain+"[v]", "-map", "[v]",
			"-frames:v", "2", "-f", "null", "-")
		return err == nil
	}

	base := "gfxcapture=monitor_idx=0"
	switch {
	case twoFrames(base + ",hwdownload,format=bgra"):
		c.HasGfxCapture, c.GfxHWFrames = true, true
	case twoFrames(base + ",format=bgra"):
		c.HasGfxCapture, c.GfxHWFrames = true, false
	default:
		c.GfxWhy = "gfxcapture is present but produced no frames on this machine"
		return
	}

	tail := ",format=bgra"
	if c.GfxHWFrames {
		tail = ",hwdownload,format=bgra"
	}
	// display_border is the one a player would notice: with it on, Windows
	// paints a yellow frame round the game for as long as capture runs. On
	// ffmpeg 9.0 it already defaults to off, and it is still passed explicitly
	// rather than relied on, because a default is somebody else's decision.
	if c.GfxOpts["display_border"] {
		c.GfxBorderOff = twoFrames(base + ":display_border=false" + tail)
	}
	if c.GfxOpts["capture_cursor"] {
		c.GfxCursorOff = twoFrames(base + ":capture_cursor=false" + tail)
	}
}
...
func buildArgs(spec captureSpec, rc recorderConfig, caps *ffmpegCaps, mode string, loop *loopbackCapture) []string {
	args := []string{"-hide_banner", "-loglevel", "warning"}

	// Both clever paths produce video from a FILTER SOURCE rather than from an
	// -i, so every downstream decision - the audio input index, the stream
	// mapping - depends on that and not on which of them is in use. Only
	// gdigrab has a real video input.
	fromFilter := mode != "gdigrab"
	...
	switch mode {
	case "gfxcapture":
		// Pointed at Siege's own window, so there is no crop and nothing else on
		// the desktop can appear. Every option below is passed ONLY if this
		// ffmpeg listed it and, for the two that change behaviour, only if it
		// was accepted when tried. See probeGfxCapture.
		opts := []string{fmt.Sprintf("hwnd=%d", spec.WindowHandle)}
		if caps.GfxOpts["max_framerate"] {
			opts = append(opts, fmt.Sprintf("max_framerate=%d", spec.FPS))
		}
		if caps.GfxCursorOff {
			opts = append(opts, "capture_cursor=false")
		}
		if caps.GfxBorderOff {
			// Windows paints a yellow frame around a captured window when this
			// is on. Every SiegeIQ player would be looking at it.
			opts = append(opts, "display_border=false")
		}
		filters = append(filters, "gfxcapture="+strings.Join(opts, ":"))
		if caps.GfxHWFrames {
			filters = append(filters, "hwdownload")
		}
		filters = append(filters, "format=bgra")
	...
	// The segment list is how a clip stops being a second out. It records each
	// segment's exact place on the capture timeline, which segindex.go anchors
	// to the wall clock. It writes a small text file beside the footage and
	// changes nothing about the video itself.
	//
	// Added only when the probe SAW this build offer it. Everything else on this
	// command line has been tested on this machine one way or another, and this
	// is not the command line to start guessing on: an ffmpeg that rejects an
	// option does not record badly, it does not record. Without the list,
	// segindex.go falls back to its own stamps, which are still far closer than
	// the filename ever was.
	if caps.HasSegmentList {
		args = append(args,
			"-segment_list", filepath.Join(spec.SegmentDir, segListName),
			"-segment_list_type", "csv",
			"-segment_list_flags", "+live",
		)
	}

	args = append(args, filepath.Join(spec.SegmentDir, segmentPattern))
	return args
}

// captureMode picks the path, preferring the one that captures the game rather
// than the screen the game is drawn on.
//
// gfxcapture wins whenever it is available and a window handle is known, and
// not only for the alt-tab behaviour: it is the only path where the desktop
// physically cannot end up in a clip, which is a promise the other two keep by
// pausing rather than by construction.
func captureMode(rc recorderConfig, caps *ffmpegCaps, spec captureSpec) string {
	if rc.CaptureMethod != "gdigrab" && rc.CaptureMethod != "ddagrab" &&
		caps.HasGfxCapture && spec.WindowHandle != 0 {
		return "gfxcapture"
	}
	if wantDDA(rc, caps, spec) {
		return "ddagrab"
	}
	return "gdigrab"
}

// captureIsWindowNative answers the recorder's question: may recording continue
// while Siege is behind another window?
//
// It deliberately does NOT read the running session: it is consulted before
// capture starts as well as during, and a session that has not begun yet must
// not be treated as "unsafe to keep filming" or the recorder would never start
// while the player is alt-tabbed.
//
// It reads captureCaps, the WARMED answer, and never probeFFmpeg. That is not a
// style choice. This is called from the recorder's own loop on every tick, and
// probeFFmpeg on a cold cache launches ffmpeg seven times while holding a mutex
// - the same shape as the bug written up above the warmed cache, which jammed
// the app window on "Connecting..." forever. Until the warm-up finishes this
// answers false, which is the safe direction: pause on alt-tab, as before.
func captureIsWindowNative(rc recorderConfig) bool {
	if rc.CaptureMethod == "gdigrab" || rc.CaptureMethod == "ddagrab" {
		return false
	}
	...
	return caps.HasGfxCapture && siegeWindowHandle() != 0
}
...
type ffmpegSession struct {
	...
	// idx watches the segment folder and records when each segment really
	// started. Owned here for the same reason as loop: it must not outlive the
	// capture it is describing.
	idx *segIndex
}
...
func (b ffmpegBackend) Start(spec captureSpec, rc recorderConfig) (captureSession, error) {
	...
	mode := captureMode(rc, caps, spec)
	if mode == "gdigrab" && spec.WindowTitle == "" {
	...
	args := buildArgs(spec, rc, caps, mode, loop)
	...
	// The segment watcher starts BEFORE ffmpeg, so every file already sitting in
	// the buffer folder is recognised as an earlier session's before ffmpeg has
	// a chance to create one of its own. Getting this order wrong would stamp
	// old footage with the current time, which is worse than not stamping it.
	s.idx = startSegIndex(spec.SegmentDir)
	...
	go func() {
		err := cmd.Wait()
		// Stopped here rather than in Stop(), because a recording can also end by
		// crashing or being killed, and the newest segment - the one most likely
		// to be wanted - deserves a precise time down every one of those paths.
		s.idx.Stop()
		s.mu.Lock()
		s.running = false
