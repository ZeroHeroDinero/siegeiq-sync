...
type config struct {
	...
	// Window appearance. Stored here rather than in the page because the page
	// is rewritten to disk on every launch, so anything it kept locally would
	// not survive an update. Empty means the shipped default, which keeps an
	// existing install looking exactly as it did. See clipedit.go.
	UITheme   string `json:"ui_theme,omitempty"`
	UIFont    string `json:"ui_font,omitempty"`
	UIDensity string `json:"ui_density,omitempty"`

	// Which clip view the window was last left on: list, grid, small, medium or
	// large. Here for the same reason as the three above - the page is rewritten
	// to disk on every launch, so it cannot remember this itself.
