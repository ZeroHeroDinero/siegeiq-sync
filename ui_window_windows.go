...
func bindUI(w webview2.WebView) {
	...
	bind("goSharePrefs", func(string) string { return jsonOf(apiSharePrefs()) })
	...
	// because the window is loaded from file:// and can open a clip directly.
	bind("goTrimClip", func(raw string) string { return apiTrimClip(raw) })
	bind("goSaveUIPrefs", func(raw string) string { return apiSaveUIPrefs(raw) })
