...
type recorderConfig struct {
	...
	// The window around a kill, for KeepKillMoments. Separate from PrePadSec and
	// PostPadSec on purpose: those absorb keyframe snap on a round-sized span, and
	// a five second moment wants its own, much tighter, shape.
	KillLeadSec  int `json:"kill_lead_sec"`  // footage kept before each kill
	KillTrailSec int `json:"kill_trail_sec"` // footage kept after each kill
	...
	// OverlayOn shows a one-line reminder over the game between rounds.
	//
	// Stored as the ON state, breaking this file's own absent-means-on convention,
	// and deliberately. That convention exists so a shipped improvement reaches
	// existing installs without asking. This one draws on top of somebody's game,
	// and a feature that starts painting over a competitive match because they
	// accepted an update is not an improvement, it is a surprise. Opt in.
	OverlayOn bool `json:"overlay_on,omitempty"`
	...
	//   gfxcapture - force the WinRT window-native path, which keeps recording
	//              while Siege is behind another window. Ignored when this
	//              machine's ffmpeg has no gfxcapture source.
...
func defaultRecorderConfig() recorderConfig {
	return recorderConfig{
		...
		// Eight seconds before and four after. A Siege kill is decided by what the
		// player did in the couple of seconds before it, so the lead is the half
		// that matters and the trail only needs to show the outcome.
		KillLeadSec:  8,
		KillTrailSec: 4,
...
func (rc *recorderConfig) normalise() {
	...
	if rc.KillLeadSec <= 0 {
		rc.KillLeadSec = d.KillLeadSec
	}
	if rc.KillTrailSec <= 0 {
		rc.KillTrailSec = d.KillTrailSec
	}
	if rc.PrePadSec < 0 {
	...
	case "auto", "ddagrab", "gdigrab", "gfxcapture":
	default:
