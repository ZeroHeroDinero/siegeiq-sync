...
// setOverlayOn turns the between-rounds reminder on or off and persists it, so the
// tray and the Settings pane can never end up disagreeing about it.
func (r *recorder) setOverlayOn(on bool) {
	r.mu.Lock()
	r.rc.OverlayOn = on
	if r.cfg != nil {
		r.cfg.Recorder = r.rc
	...
	r.mu.Unlock()
	logf("recorder: in-game reminder %s", map[bool]string{true: "on", false: "off"}[on])
}

// setTournamentActive is the writer that never existed. Until 2026-08-16 the
// field it sets was read in two places and assigned in none, so ModeTournament
// was permanently off. See tournamentwatch.go.
func (r *recorder) setTournamentActive(on bool, matchID string) {
	r.mu.Lock()
	changed := r.tournamentActive != on
	r.tournamentActive = on
	r.mu.Unlock()
	if !changed {
		return
	}
	if on {
		logf("recorder: tournament match %s is live - buffering", matchID)
	} else {
		logf("recorder: no tournament match live - tournament buffering stopped")
	}
}
...
func (r *recorder) shouldCapture() (bool, string) {
	...
	// A window-native backend captures Siege's OWN composited output rather than
	// the screen Siege happens to be drawn on. Nothing else on the desktop can
	// reach a frame, so there is nothing left to protect the player from and no
	// reason to stop when they alt-tab. That is the entire point of that path.
	//
	// noteFocusedNow is still called so focuslog stays empty: with no unfocused
	// windows recorded, excludeUnfocused has nothing to trim, and a clip is no
	// longer cut in half because somebody checked Discord in the middle of it.
	r.mu.Lock()
	rcNow := r.rc
	r.mu.Unlock()
	if captureIsWindowNative(rcNow) {
		noteFocusedNow(time.Now())
		r.mu.Lock()
	...
	if siegeInForeground() {
		...
		if r.unfocusedSince.IsZero() {
			r.unfocusedSince = now
		}
		away := now.Sub(r.unfocusedSince)
		r.mu.Unlock()
		// Whatever the grace decides, these seconds are NOT the game, and
		// focuslog.go is what stops them reaching a clip.
		noteUnfocusedNow(now)
		if away > focusGrace {
...
//
// Cut from 4s to 2s on 2026-08-16. Four seconds was chosen to absorb focus
// flicker and it does that job at two just as well, because the flickers it
// exists for (a notification, a Discord popup, the game's own alt-tab handling)
// last a fraction of a second. Four seconds of a real alt-tab, on the other
// hand, is enough to record an inbox. The grace is now the smallest value that
// still does its job rather than a comfortable round number.
//
// This only bounds how much lands in the local buffer. What actually keeps it
// out of a saved clip is focuslog.go.
const focusGrace = 2 * time.Second
...
// recorder-on/sync-off work. `ev` is the decoded per-round detail from the upload
// reply and is nil whenever there was no upload to get it from - paused, failed,
// or Verified Stats offline. Nil is a normal, handled state: every keep rule that
// needs it degrades to the action phase and says so on the clip.
func (r *recorder) handleMatch(matchFolder string, files []string, ev *matchEvents) {
	rc := r.settings()
	...
	go func() {
		...
		// Overlay the decoded detail. Rounds the backend gave real boundaries for stop
		// being estimates; rounds it did not mention keep theirs. Harmless when ev is
		// nil, so there is no branch here.
		plan.applyEvents(ev)
		...
		for _, span := range spans {
			// Never write footage that was captured while the player was looking at
			// something other than Siege. See focuslog.go.
			adj, fnote, ok := excludeUnfocused(span, time.Now())
			if !ok {
				logf("recorder: skipped %s - %s", span.Label, fnote)
				continue
			}
			if fnote != "" {
				logf("recorder: %s", fnote)
				notes = append(notes, fnote)
			}
			span = adj
